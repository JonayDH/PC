...
void print_mat(structMat* mat) { // mat = $a0
  int nFil = mat->nFil;
  int nCol = mat->nCol;
...
void change_elto(structMat* mat, int indF, int indC, double valor) { //cambia un elemento
  int numCol = mat->nCol;
  double* datos = mat->elementos;
  datos[indF * numCol + indC] = valor;  // datos[indF][indC]
}

void swap(double* e1, double* e2) { //intercambia dos elementos (intercambia 2 direcciones de memoria, aunque no sean de la misma matriz)
  double temp1 = *e1;
  double temp2 = *e2;
  *e1 = temp2;
  *e2 = temp1;
...
void intercambia(structMat* mat, int indF, int indC) { // intercambia un elemento de una matriz con su opuesto
  int numCol = mat->nCol;
  int numFil = mat->nFil;
  double* datos = mat->elementos;
  // e1 = &(datos[indF][indC]);
...
void procesa_cols(structMat* mat, int indC1, int indC2) { //compara columnas y hace cosas con ellas
  int numCol = mat->nCol;
  int numFil = mat->nFil;
  double* datos = mat->elementos;
...
double find_max(structMat* mat) { // encuentra el elemento más grande la matriz
  int numCol = mat->nCol;
  int numFil = mat->nFil;
  double* datos = mat->elementos;
  double max = datos[0];
...
int leeFila(int numFilas) { // Pide una fila
  int indFil;
  std::cin >> indFil;
...
int leeColumna(int numColumnas) { // Pide una columna
  int indCol;
  std::cin >> indCol;
...
std::tuple<int, int> pideFilaYColumna(structMat* mat) { // Pide una fila y la columna
  std::cout << "\nIndice de fila: ";
