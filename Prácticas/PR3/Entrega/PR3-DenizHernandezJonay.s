...
cadMenu:        .ascii "(0) Terminar el programa\n"
                ...
                .ascii "(2) Procesar diagonal principal\n"
...
cadFin:         .asciiz "\n\nTermina el programa\n"
IntroUmbral:	.asciiz "\nIntroduce el umbral para procesar la diagonal: "

	.text


#int procesa_diagonal(structMat* mat, double umbral) {
procesa_diagonal:
...
# push: $s0, $s1, $s2, $s3, $s4, $f20, $f22, $f24, $ra, $s5
	addi $sp,-52
	sw $s0,0($sp)
	sw $s1,4($sp)
	sw $s2,8($sp)
...
#  while (i < numFil && i < numCol) {
	procesa_diagonal_while_condicion:
	
		slt $t0,$s4,$s1
		slt $t1,$s4,$s0
		and $t2,$t0,$t1
		bnez $t2,procesa_diagonal_while_dentro
		b procesa_diagonal_while_fuera
	
	procesa_diagonal_while_dentro:
	
#    double valor = datos[i * numCol + i];  // datos[i][i]
		mul $t0,$s4,$s0
		add $t0,$t0,$s4
		mul $t0,$t0,tamD
		add $t0,$t0,$s2
...
#    if (valor > umbral) {
		procesa_diagonal_if_condicion:
		
			c.le.d $f22,$f24
			bc1f procesa_diagonal_if_dentro
			b procesa_diagonal_if_else
		
		procesa_diagonal_if_dentro:
		
#      newval = umbral;
			mov.d $f20,$f24
			b procesa_diagonal_if_fuera
			
#    }
#    else {
		procesa_diagonal_if_else:
...
#    }
		procesa_diagonal_if_fuera:

#    change_elto(mat, i, i, newval);
	move $a0,$s5
	move $a1,$s4
	move $a2,$s4
	mov.d $f12,$f20
...
#    i++;
	addi $s4,1
	b procesa_diagonal_while_condicion
#  }
	procesa_diagonal_while_fuera:

#  return contador;
	move $v0,$s3
#}

# pop: $s0, $s1, $s2, $s3, $s4, $f20, $f22, $f24, $ra, $s5
	lw $s0,0($sp)
	lw $s1,4($sp)
	lw $s2,8($sp)
	lw $s3,12($sp)
...
#        int contador;
#        contador = procesa_diagonal(matTrabajo, umbral);
			move $a0,$s0
			move $f22,$f24
			
			jal procesa_diagonal
			move $s2,$v0
