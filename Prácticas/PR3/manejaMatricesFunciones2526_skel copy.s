...
# std::tuple<double, int, int> calcula_media(structMat* mat, int indCol) {
calcula_media:
...
#   int numElementos = numFil;

#   for(int f = 0; f < numFil; f++) {
	li $s4,0
	calcula_media_for_condicion:

		blt $s4,$s2,calcula_media_for_dentro
		b calcula_media_for_fuera

	calcula_media_for_dentro:

#     suma = suma + datos[f * numCol + indCol];
		mul $t1,$s4,$s1
		add $t1,$t1,$s0
		mul $t1,$t1,tamD
		add $t1,$t1,$s3
		...
		addi $s4,1
		b calcula_media_for_condicion
#   }
	calcula_media_for_fuera:

#   double media = suma / numElementos;
	mtc1 $s2,$f4
	cvt.d.w $f4,$f4
...
#   for(int f = 0; f < numFil; f++) {
	li $s4,0
	calcula_media_for2_condicion:

		blt $s4,$s2,calcula_media_for2_dentro
		b calcula_media_for2_fuera

	calcula_media_for2_dentro:

#     double valor = datos[f * numCol + indCol];
		mul $t0,$s4,$s1
		add $t0,$t0,$s0
		mul $t0,$t0,tamD
		add $t0,$t0,$s3
...
#     if (valor > media) {
		calcula_media_if_condicion:

			c.le.d $f24,$f22
			bc1f calcula_media_if_dentro
			b calcula_media_if_else_condicion

		calcula_media_if_dentro:
...
#       std::cout << ">" << "\n";
			...
			b calcula_media_if_fuera

#     } else if (valor < media) {
		calcula_media_if_else_condicion:

			c.lt.d $f24,$f22
			bc1t calcula_media_if_else_dentro
			b calcula_media_if_else_fuera

		calcula_media_if_else_dentro:
...
#         std::cout << "<" << "\n";
		...
		calcula_media_if_else_fuera:
		calcula_media_if_fuera:
#     }

		addi $s4,1
		b calcula_media_for2_condicion
#   }
	calcula_media_for2_fuera:

#   return {media, numMayor, numMenor};
	mov.d $f0,$f22
	move $v0,$s5
	move $v1,$s6
